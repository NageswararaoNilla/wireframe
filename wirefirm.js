...
function addCards() {
    ...
    for (let i = 0; i < productsData.length; i++) {
        // console.log(i);
        let cardDetails = productsData[i];
        // console.log(cardDetails.ImageUrl);
        const li = document.createElement('li');
        li.classList.add('card-item');
        const imageEl = document.createElement('img');
        imageEl.src = cardDetails.ImageUrl;
        // imageEl.setAttribute("src", cardDetails.ImageUrl.toString());
        li.appendChild(imageEl);
        const heading = document.createElement('h1');
        heading.classList.add('card-title');
        heading.textContent = cardDetails.Name;
        li.appendChild(heading);
        cardsContainer.appendChild(li);
        const descEl = document.createElement('p');
        descEl.classList.add('card-desc');
        descEl.textContent = cardDetails.ShortDesc;
